...
def test_lambda_read_only(start_aws_proxy, cleanups, lambda_execution_role):
    ...
    # Invoke is a write operation - should NOT be proxied in read-only mode.
    # The request goes to LocalStack which doesn't have the function, so it should fail.
    with pytest.raises(ClientError) as ctx:
    ...
    # UpdateFunctionConfiguration is a write operation - should also be blocked
    with pytest.raises(ClientError) as ctx:
